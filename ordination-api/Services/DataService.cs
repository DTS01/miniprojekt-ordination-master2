...
public class DataService
{
    ...
    {
        // Find patienten og lægemidlet i databasen baseret på deres ID'er
        Patient p = db.Patienter.Find(patientId);
        Laegemiddel lm = db.Laegemiddler.Find(laegemiddelId);

        // Hvis patienten eller lægemidlet ikke findes, kastes en ArgumentNullException
        if (p == null || lm == null)
        {
            throw new ArgumentNullException();
        }

        // Opret en ny PN-ordination med de angivne parametre
        PN pn = new PN(startDato, slutDato, antal, lm);

        // Tilføj den nye ordination til patientens liste over ordinationer
        p.ordinationer.Add(pn);

        // Gem ændringerne i databasen
        db.SaveChanges();

        // Returner den oprettede PN-ordination
        return pn;
    }
    ...
    DateTime startDato, DateTime slutDato)
    {
        ...
        // Opret en ny DagligFast-ordination med de angivne parametre
        ...
        // Returner den oprettede DagligFast-ordination
        return df;
    }
    ...
    {
        // Find ordinationen i databasen baseret på dens ID
        var ordination = db.Ordinationer.Find(id);

        // Hvis ordinationen ikke findes, returneres en fejlbesked
        if (ordination == null)
        {
            return "Ordinationen blev ikke fundet.";
        }

        // Anvend ordinationen på den angivne dato
        string resultat = $"Ordination med ID {id} blev anvendt på dato {dato.dato}.";

        // Gem ændringerne i databasen
        db.SaveChanges();

        // Returner resultatet af anvendelsen af ordinationen
        return resultat;
    }
    ...
    public double GetAnbefaletDosisPerDøgn(int patientId, int laegemiddelId)
    {
        ...
        // Beregn den anbefalede dosis per døgn baseret på patientens vægt og lægemidlets enhed
        double anbefaletDosisPerDøgn = 4;
        if (p.vaegt < 25)
        {
            anbefaletDosisPerDøgn = lm.enhedPrKgPrDoegnLet * p.vaegt;
        }
        else if (p.vaegt <= 120)
        {
            anbefaletDosisPerDøgn = lm.enhedPrKgPrDoegnNormal * p.vaegt;
        }
        else
        {
            anbefaletDosisPerDøgn = lm.enhedPrKgPrDoegnTung * p.vaegt;
        }

        // Returner den beregnede anbefalede dosis per døgn
